...
def perm_add_replicas(issuer: "InternalAccount", kwargs: dict[str, Any], *, session: "Optional[Session]" = None) -> bool:
    ...
    for f in kwargs['files']:
        scope_string = f['scope']
        # To force non-admin users to only upload files in their scopes, the root user and the admin should add only scopes with format "user.<ACCOUNT>"!
        if scope_string == f'user.{issuer}': 
            return True
...
def perm_update_replicas_states(issuer: "InternalAccount", kwargs: dict[str, Any], *, session: "Optional[Session]" = None) -> bool:
    ...
    :returns: True if account is allowed, otherwise False
    """
    if _is_root(issuer) or has_account_attribute(account=issuer, key='admin', session=session) or has_account_attribute(account=issuer, key='sign-gcs', session=session):
        return True
    #else:
    #    scopes = [did['scope'] for did in kwargs['attachments']]
    #    scopes = list(set(scopes))
    #    for scope in scopes:
    ...
    #        else:
    #            return True
    #if rucio.core.scope.is_scope_owner(scope=kwargs['scope'], account=issuer, session=session):
    #    return True has_account_attribute(account=issuer, key='sign-gcs', session=session) or rucio.core.scope.is_scope_owner(scope=kwargs['scope'], account=issuer, session=session)
