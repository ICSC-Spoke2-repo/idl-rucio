...
#Add path to /usr/local/bin/ to locate IDL and cred.py from anywhere if not already present
grep -qxF "export PATH=$PATH:/usr/local/bin" ~/.bashrc || echo -e "export PATH=$PATH:/usr/local/bin\n\n" >> ~/.bashrc
...
grep -qxF 'eval "$(register-python-argcomplete IDL)"' ~/.bashrc || echo -e 'eval "$(register-python-argcomplete IDL)"' >> ~/.bashrc
